...
- (int)match:(NSArray *)otherCards //checks to see if two match but the other card doesn't in one of the four attributes.  If it does it can't be a match.  If all cards are different or same in one catagor then that method returns one.  If there is a two and one then it can't match.
{
    int score = 1;
...
//checks to see if all the shaddes are the same or all different.  Same for the other 3 methods below but they check their respective attributes.
